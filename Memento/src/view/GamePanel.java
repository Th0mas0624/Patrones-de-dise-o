...
public class GamePanel extends JPanel {
    ...
    private ImageIcon backgroundImage = new ImageIcon(
                    "Memento/src/assets/background.png"); 
    ...
    private void drawBackground(Graphics g) {
        if (backgroundImage != null) {
            ...
            g.drawImage(backgroundImage.getImage(), 0, -backgroundOffset, panelWidth, 
                        newBgHeight - backgroundOffset, null);
        }
    }
    private void drawPlayer(Graphics g) {
        if (player != null && player.getSprite() != null) {
            ...
            Image scaledPlayerImage = player.getSprite().getImage().getScaledInstance(
                                    newPlayerWidth, newPlayerHeight, Image.SCALE_DEFAULT);
            ImageIcon scaledPlayerIcon = new ImageIcon(scaledPlayerImage);
            // Ajusta la posición del jugador según el desplazamiento del fondo
            int playerY = player.getPlayerLocation().y - backgroundOffset; 
